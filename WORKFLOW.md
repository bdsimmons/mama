...
**Chapter list** — `enter` read · `w` write · `G` list gaps · `g` new gap ·
`e` editor · `p` read · `j/k` move

**Reader** — `n`/`N` next/previous gap · `enter` write here · `x` close this gap ·
`g` new gap · `e` editor · `q` back
